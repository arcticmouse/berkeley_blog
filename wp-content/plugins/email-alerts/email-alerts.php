...
Description: DO NOT UPDATE! Allows users to configure whether they wish to be alerted when a comment is added, comment held, post posted.
Version: 1.05
...
class EmailAlerts extends EmailAlerts_Plugin
{
	...
	function user_profile() {
		...
		//5-22-2012 SL: commented out if statement
	...
	function send_notifications( $subscription ) {
		...
		foreach( $this->subscriber_ids AS $subscriber_id )
		{
			...
			//5-22-2012 SL: Send for all posts but only for comments on post by subscriber
			if (($subscriber_id == $this->post->post_author && $subscription =='approve_comment') ||
				 $subscription =='publish_post')
			{
				// Subscriber name
