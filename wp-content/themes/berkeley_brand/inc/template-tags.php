...
function alienship_do_sidebar( $prefix = false ) {
	...
	if ( current_theme_supports( 'theme-layouts' ) && 'layout-1c' !== theme_layouts_get_layout() || 'footer' == $prefix || !current_theme_supports( 'theme-layouts' ) ):
		...
		// Responsive departmental footer
		if ('footer' == $prefix) {
			$count = 0;
			if ( is_active_sidebar( $prefix.'-1' )) {$count++ ;} 
			if ( is_active_sidebar( $prefix.'-2' )) {$count++ ;}  
			if ( is_active_sidebar( $prefix.'-3' )) {$count++ ;}
			if ( is_active_sidebar( $prefix.'-4' )) {$count++ ;}
			if ($count == 2) { $responsive_col = 'col-sm-6';
			} elseif ($count == 3) {$responsive_col = 'col-sm-4';
			} elseif ($count == 4) {$responsive_col = 'col-sm-3';
			} else {
				$responsive_col = 'col-sm-12';
			} ?>
			 <?php
            ...
            ?>           
			 <div class="container">
				...
				foreach ($arr as &$value) { 
					if ( is_active_sidebar( $prefix.'-'.$value ) ): 
                    	$id = $sidebars_widgets[$prefix.'-'.$value][0]; ?>              
                        <section id="<?php echo $prefix; ?>-sidebar-<?php echo $value;?>" class="sidebar widget <?php echo $responsive_col.' '.$wp_registered_widgets[$id]['classname'];?>">
                            <?php dynamic_sidebar( $prefix.'-'.$value ); ?>
                        </section>
                   <?php endif;
				 }
				?>
...
function alienship_do_accordion_title() {
	...
	if ( 0 === mb_strlen( $title ) )
		...
		$entry_title = sprintf( '<a title="%s" data-toggle="collapse" data-parent="#accordion" href="#collapse-%u" href="%s">%s</a></h4>', the_title_attribute( 'echo=0' ), $post->ID, get_permalink(), $title );

	echo apply_filters( 'alienship_accordion_title_text', $entry_title );
}
add_action( 'alienship_accordion_header', 'alienship_do_accordion_title' );
...
function alienship_do_posted_on() {
	...
	printf( __( '<span class="published-date"><em class="glyphicon glyphicon-calendar" title="Published date"></em> <a href="%1$s" title="%2$s"><time class="entry-date" datetime="%3$s">%4$s</time></a></span>', 'alienship' ),
...
function alienship_do_post_author() {
	...
	printf( __( '<span class="byline"><em class="glyphicon glyphicon-user"></em> <span class="author vcard"><a class="url fn n" href="%1$s" title="%2$s" rel="author">%3$s</a></span></span>', 'alienship' ),
...
function alienship_do_post_categories() {
	...
	if ( $post_categories ) {
		...
		echo '<span class="cat-links"><em class="glyphicon glyphicon-folder-open" title="Categories"></em> ';
...
function alienship_do_post_tags() {
	...
	if ( $post_tags ) {
		...
		echo '<span class="tags-links"><em class="glyphicon glyphicon-tags" title="Tags"></em> ';
...
function alienship_do_site_title() {
	...
	$element_before = is_front_page() || is_home() ? '<h1>' : '<a href="/" rel="home">';
	$element_after = is_front_page() || is_home() ? '</h1>' : '</a>';
	
	// Get site name format	
	if ( of_get_option( 'alienship_site-name-format' ) == "2" ) {
		$site_name_class = 'two-line-logo'; //two-line with logo
	} else if ( of_get_option( 'alienship_site-name-format' ) == "3" ) {
		$site_name_class = 'one-line-without-wordmark'; //one-line without logo
	} else {
		$site_name_class = 'one-line';  //one-line with logo
	}
	
	$navbar_div = '<div class="navbar-header '. $site_name_class. '">';

	// Title content that goes inside wrapper
	if ( of_get_option( 'custom_site_name_line_1' ) ) {
		$custom_site_name = of_get_option( 'custom_site_name_line_1' );
		if (of_get_option( 'custom_site_name_line_2' ) && $site_name_class == 'two-line-logo') {
			$custom_site_name .= '</br>'. of_get_option( 'custom_site_name_line_2' );
		}
		$site_name = sprintf( '<span class="dept-title '. $site_name_class. '">%s</span>', $custom_site_name);
	} else {
		$site_name = sprintf( '<span class="dept-title '. $site_name_class. '">%s</span>', esc_attr( get_bloginfo( 'name' ) ) );
	}
	// Adding brand workmark
	$wordmark = '<span class="b-wordmark" aria-hidden="true"></span>';

	// Put it all together
	$title = $navbar_div. $element_before . $wordmark  . $site_name . $element_after;
